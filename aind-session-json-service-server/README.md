# aind-session-json-service-server
...
REST service to run aind-metadata-mapper to build session.json files.

## Local Development

Requires docker to build and run package locally. May require mounting local path.

- Run `docker build -t aind-session-json-service-server-local:latest .`
- Run `docker run -p 5000:80 -v /local_path_to_data_folders:/docker_path:ro aind-session-json-service-server-local:latest`
